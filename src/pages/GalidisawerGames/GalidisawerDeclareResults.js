...
const GalidisawerDeclareResults = () => {
  ...
  const fetchDeclareResults = async () => {
    try {
      ...
      response.data.results.forEach(result => {
        resultsMap[result.gameName.trim().toUpperCase()] = result;
      });
  ...
  const combinedData = gameOptions.map(game => {
    const gameResult =
      declaredResultsMap[game.game_name.trim().toUpperCase()];

    return {
      key: game._id,
      gameName: game.game_name,
      openTime: moment(game.open_time, "hh:mm:ssA").format("hh:mm A"),
      jodiDigit: gameResult ? gameResult.jodiDigit : '-',
      _id: gameResult ? gameResult._id : null,
      date: gameResult ? gameResult.date : null
    };
  });
